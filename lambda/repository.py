...
class ActivitiesRepo:
    ...
    def get_category_ids(self, user_id='stevebot') -> list[str]:
        ...
        return sorted({item[CATEGORY_ID] for item in response[ITEMS]})

    def get_category(self, user_id='stevebot', category_id='') -> list[Activity]:
        response = self.table.query(
            KeyConditionExpression=(Key('PK').eq(f'USER#{user_id}') & Key('SK').begins_with(f'CATEGORY#{category_id}#'))
        )
        return sorted([Activity.from_dict(item) for item in response[ITEMS]],
